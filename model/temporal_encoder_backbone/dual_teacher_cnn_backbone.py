...
def main():
    ...
    opt_b_a = Adam(list(attr_model.backbone.parameters()) + list(cnn.parameters()), lr=lr_b_a)
    ...
    for epoch in range(1, epochs+1):
        ...
        loss_ds = F.cross_entropy(fs[valid_idx], struct_labels_train.view(-1)[valid_idx])
        ...
        emb_a_seq = emb_a.permute(1,2,0)
        agg_emb_a_seq = cnn(emb_a_seq)                   # [N, 64, T]
        agg_emb_a_seq = agg_emb_a_seq.permute(2,0,1)      # [T, N, 64]
        ...
        num_items = emb_a.size(1) - num_users

        loss_lp = 0.0
        for t in range(agg_emb_a_seq.size(0)-1):
            z = agg_emb_a_seq[t]
            pos = train_graphs_attr[t+1].edge_index.to(device)
            bi_pos = pos.clone(); bi_pos[1] -= num_users
            neg_bi = negative_sampling(
                edge_index=bi_pos,
                num_nodes=(num_users, num_items),
                num_neg_samples=bi_pos.size(1),
            ).to(device)
        ...
        loss_lp /= (agg_emb_a_seq.size(0)-1)

        opt_b_s.zero_grad(); opt_b_a.zero_grad()
        total_struct = loss_adv_s + beta_cd * loss_cd
        total_attr   = loss_adv_a + beta_cd * loss_cd + gamma_lp * loss_lp
        total_struct.backward(retain_graph=True)
        total_attr.backward()
        opt_b_s.step(); opt_b_a.step()
    ...
    # torch.save(struct_model, out_model_dir / 'structural_el_cnn.pth')
    # torch.save(attr_model,   out_model_dir / 'attribute_el_cnn.pth')
